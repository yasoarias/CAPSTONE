      ...
      <form @submit.prevent="startRegistration">
        ...
        <div class="form-group">
          <input
            ...
            required
          />
          <div class="validation-message" v-if="emailError" :class="{ error: true }">
            {{ emailError }}
          </div>
          <div class="validation-message" v-if="isDisposableEmail" :class="{ error: true }">
            Please use a permanent email address
          </div>
          <div class="validation-message" v-if="emailSuccess" :class="{ success: true }">
            Valid email address
          </div>
        </div>
...
const isDisposableEmail = ref(false);
...
const handleEmailInput = () => {
  emailError.value = '';
  isDisposableEmail.value = false;
  emailSuccess.value = false;
};
// Validate email - Abstract API
const validateEmail = async () => {
  if (!email.value || !email.value.includes('@') || email.value.split('@')[1].length < 2) {
    emailError.value = '';
    isDisposableEmail.value = false;
    emailSuccess.value = false;
    return;
  ...
  try {
    const API_KEY = 'f3adc6779bcf4764b5c70abf9579581b';
    const response = await fetch(`https://emailvalidation.abstractapi.com/v1/?api_key=${API_KEY}&email=${encodeURIComponent(email.value)}`);
    ...
    if (result.deliverability === "UNDELIVERABLE") {
      ...
      isDisposableEmail.value = true;
      emailError.value = 'Please use a permanent email address';
      emailSuccess.value = false;
    } else {
      emailError.value = '';
      isDisposableEmail.value = false;
    ...
    emailError.value = 'Unable to verify email at this time';
    emailSuccess.value = false;
  }
};
</script>
