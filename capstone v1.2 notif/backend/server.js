...
app.post('/api/bookings', verifyToken, async (req, res) => {
  ...
  try {
    ...
    const bookingResult = await db.query(
      'INSERT INTO bookings (user_id, event_type, venue_name, status) VALUES (?, ?, ?, "Pending")',
      [userId, bookingData.eventType, bookingData.venueName]
    );
    
    // Create admin notification
    await db.query(
      'INSERT INTO admin_notifications (booking_id, message, type, created_at) VALUES (?, ?, "booking_request", NOW())',
      [bookingResult.insertId, `New booking request for ${bookingData.eventType}`]
    );
...
// Create user notification when admin updates booking status
app.put('/api/bookings/:id/status', verifyToken, isAdmin, async (req, res) => {
  const { id } = req.params;
  const { status } = req.body;
  
  try {
    const [booking] = await db.query('SELECT user_id FROM bookings WHERE id = ?', [id]);
